...
const ChallengeComponent = ({ nextState }: ComponentsProps): JSX.Element => {
    ...
    return (
        <div className="w-full h-full grid grid-cols-2 lg:grid-rows-7 grid-rows-6 gap-x-4 gap-y-2">
            <div className="lg:col-span-1 lg:row-start-1 lg:row-end-5 col-span-2 row-start-1 row-end-2">
                <div>
                    <CronoComponent
            ...
            <div className="lg:col-span-1 lg:col-start-1 lg:row-start-5 lg:row-end-5 col-span-2 row-start-2 row-end-2 text-yellow-text text-center text-4xl">
                {`SCORE : ${puntuation}`}
            </div>
            <div className="lg:col-span-1 lg:row-span-1 lg:col-start-1 lg:row-start-7 col-span-2 row-span-1 row-start-6">
                <ToxicTextComponent loading={false} asyncSubmit={sendInferenceAndUpdateResult} />
            </div>
            <div
                ref={historyLogRef}
                className="bg-yellow-text bg-opacity-30 rounded p-4 text-2xl text-yellow-text overflow-y-scroll scroll-smooth lg:col-span-1 lg:row-span-7 col-span-2 row-span-3 row-start-3 row-end-6"
